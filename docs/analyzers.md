...
- **shallow-module** only fires on `export`ed top-level functions whose body is a single return-of-call, a single call statement, or a `const x = call(); return x` pair. Suppress with a `// cerberus-allow: shallow-module` line comment when the indirection is intentional (testing seam, public API stability).
